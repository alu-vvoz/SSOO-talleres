...
{
	unsigned int group_index = blockgroup_for_inode(inode_number);
	unsigned int inode_index = blockgroup_inode_index(inode_number);
	
	//TODO: Ejercicio 2
	// Buscar grupo
	//unsigned int index_inode_in_group = (inode_number - 1) % _superblock->inodes_per_group;
	
	// Buscar bloque
	unsigned int cant_inodos_por_bloque = BLOCK_SIZE / _superblock->inode_size;
	unsigned int indice_block_en_tabla = inode_index / cant_inodos_por_bloque;

	// Buscar inodo en tabla de inodo
	unsigned int index_in_block = inode_index % cant_inodos_por_bloque;

	unsigned char * bloque = (unsigned char*) malloc(BLOCK_SIZE*sizeof(char));
	Ext2FSBlockGroupDescriptor* desc =  block_group(group_index);

	read_block(desc->inode_table + indice_block_en_tabla, bloque);
	Ext2FSInode * res = (Ext2FSInode*) malloc(_superblock->inode_size);
	
	memcpy(res, bloque + index_in_block * sizeof(Ext2FSInode), _superblock->inode_size);
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	//TODO: Ejercicio 1
	unsigned int* blocks = inode->block;
	const unsigned int cant_entradas_primera = 256;
	const unsigned int cant_entradas_segunda = 256 * 256;
	const unsigned int cant_entradas_tercera = 256 * 256 * 256;

	unsigned int res;
	if (block_number < 12) {
		res = blocks[block_number];
		
	} else if (block_number < cant_entradas_primera){
		block_number -= 12;
		unsigned char * bloque = (unsigned char*) malloc(BLOCK_SIZE*sizeof(char));
		read_block(blocks[12], bloque);
		unsigned int* bloque2 = (unsigned int*) bloque;
		res = bloque2[block_number ];
		
	} else if (block_number < cant_entradas_segunda) {
		block_number -= cant_entradas_primera + 12;
		
		unsigned char * bloque = (unsigned char*) malloc(BLOCK_SIZE*sizeof(char));

		read_block(blocks[13], bloque);
		unsigned int indice_primera_indir = block_number / cant_entradas_primera;
		unsigned int indice_segunda_indir = block_number % cant_entradas_primera;
		unsigned int* bloque2 = (unsigned int*) bloque;
		unsigned int segunda_tabla = bloque2[indice_primera_indir];

		read_block(segunda_tabla, bloque);
		unsigned int res = bloque2[indice_segunda_indir];

	} else {
		block_number -= cant_entradas_segunda + cant_entradas_primera + 12;
		
		unsigned char * bloque = (unsigned char*) malloc(BLOCK_SIZE*sizeof(char));

		read_block(blocks[14], bloque);
		unsigned int* bloque2 = (unsigned int*) bloque;
		unsigned int indice_primera_indir = block_number / cant_entradas_segunda;
		unsigned int indice_segunda_indir = (block_number % cant_entradas_segunda) / cant_entradas_primera;
		unsigned int indice_tercera_indir = (block_number % cant_entradas_segunda) % cant_entradas_primera;

		unsigned int segunda_tabla = bloque2[indice_primera_indir];
		read_block(segunda_tabla, bloque);
		unsigned int tercera_tabla = bloque2[indice_segunda_indir];
		read_block(tercera_tabla, bloque);
		unsigned int res = bloque2[indice_tercera_indir];
	}
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	const unsigned int bloques_totales = from->blocks;
	unsigned char * bloque = (unsigned char*) malloc(BLOCK_SIZE*sizeof(char));
	//TODO: Ejercicio 3
	for (unsigned int i = 0; i < bloques_totales; i++){
		unsigned int bloque_dir = get_block_address(from,i);
		read_block(bloque_dir, bloque);
		unsigned int offset = 0;
		unsigned int var = 1;
		while (offset < BLOCK_SIZE && var != 0) {
			Ext2FSDirEntry* dir_entry = (Ext2FSDirEntry*) bloque + offset;
			unsigned int salto = dir_entry->record_length;
			unsigned int tipo = dir_entry->file_type;
			std::cout<<tipo<<","<<EXT2_FT_REG_FILE<<std::endl;
			unsigned int largo_nombre = dir_entry->name_length;
			if (tipo == EXT2_FT_REG_FILE){

				
				char* nombre = (char*)malloc(largo_nombre);
				char* acopiar = (char*) &dir_entry->name;

				strcpy(nombre, acopiar) ;

				if (strcmp(nombre, filename) == 0) {
